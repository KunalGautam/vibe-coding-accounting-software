...
## Recently Completed
- Broadened Flutter offline write replay beyond expense drafts to cover invoice drafts, attachment metadata creation, and manual investment price capture with shared retry/error/conflict handling.
...
- Flutter: offline-ready expense/invoice/investment shell with file-backed queues/caches, typed API transport, conflict-aware queued writes for expenses/invoices/attachment metadata/investment prices, and cached read models.
...
- Offline sync depth: Flutter queued writes now persist retry/error/conflict metadata, surface conflict review state, and replay expense drafts, invoice drafts, attachment metadata, and investment prices; native SQLite persistence, binary attachment upload replay, and additional module write queues remain.
...
1. Flutter SQLite cache migration for file-backed repositories and durable attachment blob metadata.
2. Broader offline write queues for payments, invoice/expense edits, bank import drafts, and approval/status transitions.
