...
- Security hardening: public auth/bootstrap rate limiting, optional TOTP MFA with encrypted secret storage and one-time recovery codes, refresh-token session revocation, React password reset request/confirm UX with reset-link token detection/password guidance, MFA recovery-code copy/download, tenant isolation tests, and permission matrix tests are implemented; broader account-management UX and test polish remains.
- Email/account flows: password reset SMTP delivery, organization invitation emails with React invite-status summary, local temporary-password generation/copy/download, role guidance, and gated self-service registration are implemented; richer onboarding flows and account lifecycle controls remain.
