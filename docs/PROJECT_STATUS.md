...
- Flutter: offline-ready expense/invoice/investment/report shell with SQLite-backed sync queue/settings/account cache/customer-vendor party cache/tax catalog cache/invoice cache/investment cache/financial, tax, and budget report cache, statement/aging/tax prior-period comparison, selected-vs-previous budget comparison, local CSV export app-storage/Downloads/share-sheet support/attachment metadata cache/downloaded binary cache/queued-attachment upload manifest, typed API transport, file picker/gallery/camera attachment capture, conflict-aware queued writes for expense/invoice creation and draft edits, customer payments, vendor payments, ledger posting actions, estimate statuses/conversions, purchase-order statuses/conversions, structured/QIF/OFX bank imports, attachment metadata/binary attachment uploads, investments-page lot creation, manual price capture, dividend capture, corporate action capture, specific-lot sale queueing, average-cost sale queueing, broker holdings paste/file import queueing, and cached read models.
