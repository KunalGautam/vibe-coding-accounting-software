...
- Added Alpha Vantage daily CSV imports for API, React, and scheduled worker flows; NSE/BSE-style equity CSV imports are also available through REST, React, OpenAPI/Postman, and worker paths, with coverage aligned at 152 route/method pairs after account-lifecycle updates.
...
- Security hardening: public auth/bootstrap rate limiting, optional TOTP MFA with encrypted secret storage and one-time recovery codes, refresh-token session revocation, React password reset request/confirm UX with reset-link token detection/password guidance, MFA recovery-code copy/download, account lifecycle RBAC coverage, tenant isolation tests, permission matrix tests, and last-active-admin protection are implemented; broader account-management UX and test polish remains.
- Email/account flows: password reset SMTP delivery, organization invitation emails with React invite-status summary, local temporary-password generation/copy/download, role guidance, role edits, activate/deactivate controls, and gated self-service registration are implemented; richer onboarding journeys remain.
