...
- React web: broad admin/control surfaces, offline draft queues, cached read-only snapshots, report CSV exports, and account drilldown review from generated reports with source-document module actions.
...
- Export/reporting polish: expanded core report PDFs, backend Excel-compatible CSV downloads, account-level report drilldown with React report-row actions and source-document references, scheduled report SMTP delivery, Flutter statement/aging/tax prior-period comparison, selected-vs-previous budget comparison, and Flutter cached-report CSV generation with app-storage/Downloads/share-sheet support are implemented; richer focused document highlighting remains.
- UI polish: complete CRUD flows, validation UX, module dashboards, broader mobile/desktop Flutter parity.

## Suggested Next Build Order
1. Richer focused document highlighting after account drilldown module navigation, including selected invoice/bill/payment/payroll row emphasis.
