...
- Flutter: offline-ready expense/invoice/investment shell with SQLite-backed sync queue/settings/account cache/tax catalog cache/invoice cache/investment cache/attachment metadata cache/downloaded binary cache/queued-attachment upload manifest, typed API transport, conflict-aware queued writes for expense/invoice creation and draft edits, customer payments, vendor payments, ledger posting actions, estimate statuses/conversions, purchase-order statuses/conversions, structured/QIF/OFX bank imports, attachment metadata/binary attachment uploads, investment prices, average-cost investment sales, and cached read models.
...
1. Flutter file picker/camera polish for binary attachments and receipt capture.
