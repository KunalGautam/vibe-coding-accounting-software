...
- Added SQLite-backed Flutter offline persistence for the pending sync-operation queue and queued-attachment upload manifest, with memory/file repositories retained for tests and migration fallback.
...
- Flutter: offline-ready expense/invoice/investment shell with SQLite-backed sync queue and queued-attachment upload manifest, file-backed read caches/settings/binary cache, typed API transport, conflict-aware queued writes for expenses/invoices/customer payments/vendor payments/ledger posting actions/estimate statuses/purchase-order statuses/attachment metadata/binary attachment uploads/investment prices/average-cost investment sales, and cached read models.
...
- Offline sync depth: Flutter queued writes now persist retry/error/conflict metadata in SQLite, surface conflict review state, track queued attachment upload blob metadata in a SQLite manifest, and replay expense drafts, invoice drafts, customer payments, vendor payments, invoice/expense/bill/credit-note posting actions, estimate statuses, purchase-order statuses, attachment metadata, binary attachment uploads, investment prices, and average-cost investment sales; migration of read caches/settings/binary cache to SQLite and additional module write queues remain.
...
1. Continue Flutter SQLite migration for file-backed read caches/settings/binary cache now that the sync queue and queued-attachment upload manifest are SQLite-backed.
