...
## Recently Completed
- Added `sha256sum`-style backup checksum sidecars for primary and mirrored backup snapshots, retention/cleanup handling for sidecars, and restore CLI support for `-sha256-file` preflight verification.
...
- Production deployment: Docker/compose, explicit GORM migration CLI, backup restore CLI with checksum or sidecar-file verification and dry-run preflight, non-root app containers with dropped capabilities, liveness/readiness probes with Compose healthchecks, API server timeouts with graceful shutdown, backup `.sha256` sidecars, checksum-verified backup mirroring to a mounted second target, production environment validation, structured logging, basic Prometheus metrics, Prometheus scrape/rule config, Alertmanager email routing template, Grafana datasource/dashboard provisioning, and managed-cloud deployment/rollback/backup/incident runbook are implemented.
