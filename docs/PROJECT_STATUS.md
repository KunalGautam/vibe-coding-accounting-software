...
- Added Flutter Trial Balance, P&L, Balance Sheet, Cash Flow, AR Aging, AP Aging, Tax Liability, Tax Summary, and Budget vs Actual report transport, SQLite-backed report cache with migration support, local CSV export generation/file-save support, and a Reports page for offline financial/tax/budget snapshot review.
...
- Flutter: offline-ready expense/invoice/investment/report shell with SQLite-backed sync queue/settings/account cache/customer-vendor party cache/tax catalog cache/invoice cache/investment cache/financial, tax, and budget report cache with local CSV export file-save support/attachment metadata cache/downloaded binary cache/queued-attachment upload manifest, typed API transport, file picker/gallery/camera attachment capture, conflict-aware queued writes for expense/invoice creation and draft edits, customer payments, vendor payments, ledger posting actions, estimate statuses/conversions, purchase-order statuses/conversions, structured/QIF/OFX bank imports, attachment metadata/binary attachment uploads, investment prices, average-cost investment sales, and cached read models.
...
- Export/reporting polish: core statement PDFs, scheduled report SMTP delivery, and Flutter cached-report CSV generation/file-save support are implemented; platform share-sheet/Downloads UX, broader PDF/Excel exports, and comparative reports remain.
- UI polish: complete CRUD flows, validation UX, module dashboards, broader mobile/desktop Flutter parity.

## Suggested Next Build Order
1. Broaden Flutter report parity to comparative periods and platform share-sheet/Downloads UX for saved CSV exports.
