...
- Added SQLite-backed Flutter offline persistence for the pending sync-operation queue, queued-attachment upload manifest, sync settings, account read cache, tax catalog cache, invoice cache, investment cache, and attachment metadata cache, with memory/file repositories retained for tests and migration fallback.
...
- Flutter: offline-ready expense/invoice/investment shell with SQLite-backed sync queue/settings/account cache/tax catalog cache/invoice cache/investment cache/attachment metadata cache/queued-attachment upload manifest, file-backed binary cache, typed API transport, conflict-aware queued writes for expenses/invoices/customer payments/vendor payments/ledger posting actions/estimate statuses/purchase-order statuses/attachment metadata/binary attachment uploads/investment prices/average-cost investment sales, and cached read models.
...
- Offline sync depth: Flutter queued writes and sync settings now persist in SQLite, surface conflict review state, track queued attachment upload blob metadata in a SQLite manifest, cache accounts and tax catalog snapshots in SQLite, and replay expense drafts, invoice drafts, customer payments, vendor payments, invoice/expense/bill/credit-note posting actions, estimate statuses, purchase-order statuses, attachment metadata, binary attachment uploads, investment prices, and average-cost investment sales; migration of the remaining binary cache to SQLite and additional module write queues remain.
...
1. Continue Flutter SQLite migration for the attachment binary cache now that the sync queue, sync settings, account cache, tax catalog cache, invoice cache, investment cache, attachment metadata cache, and queued-attachment upload manifest are SQLite-backed.
