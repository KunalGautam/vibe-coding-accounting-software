...
Flutter currently has an offline-ready expense/invoice/investment shell with file-backed caches/queues, typed API transport, attachment handling, sync settings, and tests. The sync coordinator can now replay queued expense drafts, invoice drafts, attachment metadata creation, and manual investment price captures with shared retry/error/conflict metadata.
...
5. Offline sync depth: native Flutter SQLite persistence, binary attachment upload replay, and additional write queues for payments/status changes/import drafts.
...
Continue offline depth by replacing Flutter file-backed repositories with SQLite-backed repositories and adding durable blob metadata for offline attachment uploads. Keep the current operation keys (`expenses.create_draft`, `invoices.create_draft`, `attachments.create_metadata`, `investments.create_price`) and conflict metadata fields (`retry_count`, `last_attempt_at`, `last_error`, `conflict_reason`) as the sync-state contract while migrating persistence.
