...
Flutter currently has an offline-ready expense/invoice/investment shell with SQLite-backed pending sync operations, account cache, tax catalog cache, invoice cache, investment cache, and queued-attachment upload manifests, plus file-backed attachment metadata/settings/binary cache, typed API transport, attachment handling, sync settings, and tests. The sync coordinator can now replay queued expense drafts, invoice drafts, customer payments, vendor payments, invoice/expense/bill/credit-note posting actions, estimate status transitions, purchase-order status transitions, attachment metadata creation, binary attachment uploads, manual investment price captures, and average-cost investment sales with shared retry/error/conflict metadata. Queued local attachment uploads record operation IDs, local file paths, file names, sizes, timestamps, and optional content types.
...
5. Offline sync depth: continue native Flutter SQLite persistence for attachment metadata cache, attachment binary cache, and sync settings; add write queues for edits/import drafts/remaining approval transitions.
...
Continue offline depth by replacing remaining Flutter file-backed repositories with SQLite-backed repositories. `SyncOperationRepository`, `AccountCacheRepository`, `TaxCatalogCacheRepository`, `InvoiceCacheRepository`, `InvestmentCacheRepository`, and `AttachmentUploadManifestRepository` now default to SQLite, while attachment metadata cache, sync settings, and binary cache still need migration. Keep the current operation keys (`expenses.create_draft`, `invoices.create_draft`, `payments.record_customer`, `payments.record_vendor`, `ledger.post_invoice`, `ledger.post_expense`, `ledger.post_bill`, `ledger.post_credit_note`, `commercial_documents.update_estimate_status`, `commercial_documents.update_purchase_order_status`, `attachments.create_metadata`, `attachments.upload_binary`, `investments.create_price`, `investments.sell_average_cost`) and conflict metadata fields (`retry_count`, `last_attempt_at`, `last_error`, `conflict_reason`) as the sync-state contract while migrating persistence.
