...
React web currently has broad admin/control surfaces, password reset request/confirm UX, organization-user onboarding helper with temporary-password generation and invite delivery status, offline-oriented localStorage snapshots, manual draft queues, report CSV exports, report-row account drilldowns with an inline ledger movement panel, source-document module actions, focused-row highlighting, invoice/bill detail review, invoice/bill payment history tables, payroll preview/payslip flows, and typed API support.
