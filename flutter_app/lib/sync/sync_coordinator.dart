...
final Map<String, _SyncHandler> _syncHandlers = {
  ...
  'investments.create_lot': (client, operation) async {
    await client.syncInvestmentLot(operation);
  },
  'investments.import_broker_holdings': (client, operation) async {
    await client.syncBrokerHoldingsPriceImport(operation);
  },
  'investments.sell_average_cost': (client, operation) async {
    await client.syncAverageCostSale(operation);
  },
  'investments.sell_lot': (client, operation) async {
    await client.syncInvestmentLotSale(operation);
  },
