...
- Local SQLite cache for Flutter is still the target persistence engine; current repositories, including the sync queue and queued-attachment upload manifest, remain file-backed until the SQLite migration lands.
- Initial offline write replay supports expense drafts, invoice drafts, customer payments, vendor payments, ledger posting actions, estimate status changes, purchase-order status changes, attachment metadata, binary attachment uploads with a durable queued-upload manifest, manual investment prices, and average-cost investment sales; next workflows should prioritize edits, import drafts, remaining approval transitions, and native SQLite persistence.
- Platform features:
...
- Flutter app is scaffolded for Android, iOS, Linux, macOS, and Windows with offline-ready expense capture, draft expense form including cached receipt attachment ID selection, tax rate/group metadata, and tax preview, pending draft list with local edit/delete, retry/error/conflict metadata for queued sync operations, cached account lookup, cached invoice line/subtotal/tax/total/PDF-metadata viewing, cached investment lot/realized-gains/price/valuation viewing, cached attachment metadata and downloaded binary lookup, cached tax config lookup, file-backed sync settings, a shared Dart draft sync queue, durable queued-attachment upload manifest, file-backed queue/cache repositories, typed API client for accounts/invoices/expenses/attachments/tax config/investments plus attachment binary upload/download and valuation refresh, attachment lookup/sample upload/local-file upload/download/inspect UI with offline availability status, account lookup with one-tap posting account selection, tax lookup with one-tap default rate/group selection, sync coordinator for expense draft/invoice draft/customer payment/vendor payment/ledger posting/estimate status/purchase-order status/attachment metadata/binary attachment upload/investment price/average-cost sale retries and conflict review state, credential-gated live sync, and sync status UI; native SQLite persistence and polished Flutter file picker/camera UI for binary attachments are still pending; `flutter analyze` and `flutter test` pass.
