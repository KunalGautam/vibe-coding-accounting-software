...
- Data export/backups: Admin/Accountant tenant-scoped JSON export, manual local backup snapshots, scheduled worker backups, checksum metadata, optional checksum-verified mounted mirror copies, and retention pruning are implemented; native object-storage drivers are still pending.
...
- Admin/Accountant organization JSON data export is implemented at `/data/export`; local manual/scheduled backup snapshots are implemented at `/data/backups`; optional `BACKUP_MIRROR_PATH` copies each backup to a second mounted target with checksum verification; backup restore is implemented via `backend/cmd/restore` with optional `-sha256` verification and `-dry-run` preflight; native object-storage drivers are still pending.
