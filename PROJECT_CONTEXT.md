...
- Local SQLite cache for Flutter is now active for the pending sync-operation queue, sync settings, account cache, customer/vendor party cache, tax catalog cache, invoice cache, investment cache, report cache, attachment metadata cache, downloaded attachment binary cache, and queued-attachment upload manifest; cached reports can also be written as CSV files to app storage.
