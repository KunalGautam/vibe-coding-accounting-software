...
Security note: optional TOTP MFA now includes encrypted secret storage, hashed one-time recovery codes, login fallback with single-use consumption, authenticated recovery-code regeneration, React admin controls, React password reset request/confirm UX with reset-link token detection/password guidance, MFA recovery-code copy/download, and OpenAPI/Postman coverage. Remaining auth hardening is broader account-management UX and test polish.
