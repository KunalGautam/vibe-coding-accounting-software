...
func TestConfigValidateRejectsProductionDefaults(t *testing.T) {
	cfg := Config{
		...
		SecurityHSTSMaxAgeSeconds: -1,
		APIReadTimeoutSeconds:     0,
		APIWriteTimeoutSeconds:    0,
		APIIdleTimeoutSeconds:     0,
		APIShutdownTimeoutSeconds: 0,
...
func TestConfigValidateAllowsHardenedProduction(t *testing.T) {
	cfg := Config{
		...
		SecurityHSTSMaxAgeSeconds: 31536000,
		APIReadTimeoutSeconds:     15,
		APIWriteTimeoutSeconds:    30,
		APIIdleTimeoutSeconds:     120,
		APIShutdownTimeoutSeconds: 15,
		AttachmentMaxUploadBytes:  25 * 1024 * 1024,
...
func TestConfigValidateMarketDataImportSettings(t *testing.T) {
	...
	cfg.MarketDataImportFormat = "axisdirect_holdings_csv"
	if err := cfg.ValidateRuntime(); err != nil {
		t.Fatalf("ValidateRuntime() error = %v, want nil for Axis Direct holdings CSV", err)
	}

	cfg.MarketDataImportFormat = "sbisecurities_holdings_csv"
	if err := cfg.ValidateRuntime(); err != nil {
		t.Fatalf("ValidateRuntime() error = %v, want nil for SBI Securities holdings CSV", err)
	}

	cfg.MarketDataImportFormat = "nuvama_holdings_csv"
	if err := cfg.ValidateRuntime(); err != nil {
		t.Fatalf("ValidateRuntime() error = %v, want nil for Nuvama holdings CSV", err)
	}

	cfg.MarketDataImportFormat = "geojit_holdings_csv"
	if err := cfg.ValidateRuntime(); err != nil {
		t.Fatalf("ValidateRuntime() error = %v, want nil for Geojit holdings CSV", err)
	}

	cfg.MarketDataImportFormat = "iiflsecurities_holdings_csv"
	if err := cfg.ValidateRuntime(); err != nil {
		t.Fatalf("ValidateRuntime() error = %v, want nil for IIFL Securities holdings CSV", err)
	}

	cfg.MarketDataImportFormat = "fyers_holdings_csv"
	if err := cfg.ValidateRuntime(); err != nil {
		t.Fatalf("ValidateRuntime() error = %v, want nil for FYERS holdings CSV", err)
	}

	cfg.MarketDataImportFormat = "edelweiss_holdings_csv"
	if err := cfg.ValidateRuntime(); err != nil {
		t.Fatalf("ValidateRuntime() error = %v, want nil for Edelweiss holdings CSV", err)
	}

	cfg.MarketDataImportFormat = "aliceblue_holdings_csv"
	if err := cfg.ValidateRuntime(); err != nil {
		t.Fatalf("ValidateRuntime() error = %v, want nil for Alice Blue holdings CSV", err)
	}

	cfg.MarketDataImportFormat = "samco_holdings_csv"
	if err := cfg.ValidateRuntime(); err != nil {
		t.Fatalf("ValidateRuntime() error = %v, want nil for Samco holdings CSV", err)
	}

	cfg.MarketDataImportFormat = "choice_holdings_csv"
	if err := cfg.ValidateRuntime(); err != nil {
	...
	cfg.MarketDataImportFormat = "jainam_holdings_csv"
	if err := cfg.ValidateRuntime(); err != nil {
		t.Fatalf("ValidateRuntime() error = %v, want nil for Jainam holdings CSV", err)
	}
}
