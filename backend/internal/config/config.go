...
func (c Config) validate(runtime bool) error {
	...
	if c.MarketDataImportEnabled {
		...
		if !isSupportedMarketDataFormat(c.MarketDataImportFormat) {
			problems = append(problems, errors.New("MARKET_DATA_IMPORT_FORMAT must be amfi, csv, nse_equity_csv, or yahoo_finance_csv"))
		}
...
func isSupportedMarketDataFormat(format string) bool {
	switch format {
	case "amfi", "csv", "nse_equity_csv", "yahoo_finance_csv":
		return true
	default:
		return false
	}
