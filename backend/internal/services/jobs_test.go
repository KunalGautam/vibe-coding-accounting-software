...
func TestJobServiceImportScheduledMarketDataSupportsIIFLSecuritiesHoldingsCSV(t *testing.T) {
	db := testDB(t)
	ctx := context.Background()
	org := domain.Organization{Name: "Acme IIFL Securities", BaseCurrency: "INR", CountryCode: "IN", FiscalYearStartMonth: 4}
	if err := db.Create(&org).Error; err != nil {
		t.Fatalf("create organization: %v", err)
	}

	tmpDir := t.TempDir()
	path := filepath.Join(tmpDir, "iiflsecurities.csv")
	if err := os.WriteFile(path, []byte("Symbol,ISIN,Date,LTP,Quantity\nTITAN,INE280A01028,2026-07-31,3520.15,2\n"), 0o600); err != nil {
		t.Fatalf("write IIFL Securities CSV: %v", err)
	}

	result, err := NewJobService(db).ImportScheduledMarketData(ctx, MarketDataImportJobInput{
		OrganizationID: org.ID,
		Path:           path,
		Format:         "iiflsecurities_holdings_csv",
	})
	if err != nil {
		t.Fatalf("ImportScheduledMarketData() error = %v", err)
	}
	...
	var price domain.InvestmentPrice
	if err := db.Where("organization_id = ? AND symbol = ?", org.ID, "TITAN").First(&price).Error; err != nil {
		t.Fatalf("load IIFL Securities price: %v", err)
	}
	if price.PriceMinor != 352015 || price.Source != "iiflsecurities_holdings_csv" {
		t.Fatalf("unexpected IIFL Securities price: %+v", price)
	}
}

func TestJobServiceImportScheduledMarketDataSupportsFYERSHoldingsCSV(t *testing.T) {
	db := testDB(t)
	ctx := context.Background()
	org := domain.Organization{Name: "Acme FYERS", BaseCurrency: "INR", CountryCode: "IN", FiscalYearStartMonth: 4}
	if err := db.Create(&org).Error; err != nil {
		t.Fatalf("create organization: %v", err)
	}

	tmpDir := t.TempDir()
	path := filepath.Join(tmpDir, "fyers.csv")
	if err := os.WriteFile(path, []byte("Symbol,ISIN,Date,LTP,Quantity\nSBIN,INE062A01020,2026-07-31,820.45,8\n"), 0o600); err != nil {
		t.Fatalf("write FYERS CSV: %v", err)
	}

	result, err := NewJobService(db).ImportScheduledMarketData(ctx, MarketDataImportJobInput{
		OrganizationID: org.ID,
		Path:           path,
		Format:         "fyers_holdings_csv",
	})
	if err != nil {
		t.Fatalf("ImportScheduledMarketData() error = %v", err)
	}
	...
	if price.PriceMinor != 82045 || price.Source != "fyers_holdings_csv" {
		t.Fatalf("unexpected FYERS price: %+v", price)
	}
}

func TestJobServiceImportScheduledMarketDataSupportsEdelweissHoldingsCSV(t *testing.T) {
	db := testDB(t)
	ctx := context.Background()
	org := domain.Organization{Name: "Acme Edelweiss", BaseCurrency: "INR", CountryCode: "IN", FiscalYearStartMonth: 4}
	if err := db.Create(&org).Error; err != nil {
		t.Fatalf("create organization: %v", err)
	}

	tmpDir := t.TempDir()
	path := filepath.Join(tmpDir, "edelweiss.csv")
	if err := os.WriteFile(path, []byte("Symbol,ISIN,Date,LTP,Quantity\nEDELWEISS,INE532F01054,2026-07-31,910.25,4\n"), 0o600); err != nil {
		t.Fatalf("write Edelweiss CSV: %v", err)
	}

	result, err := NewJobService(db).ImportScheduledMarketData(ctx, MarketDataImportJobInput{
		OrganizationID: org.ID,
		Path:           path,
		Format:         "edelweiss_holdings_csv",
	})
	if err != nil {
		t.Fatalf("ImportScheduledMarketData() error = %v", err)
	}
	...
	var price domain.InvestmentPrice
	if err := db.Where("organization_id = ? AND symbol = ?", org.ID, "EDELWEISS").First(&price).Error; err != nil {
		t.Fatalf("load Edelweiss price: %v", err)
	}
	if price.PriceMinor != 91025 || price.Source != "edelweiss_holdings_csv" {
		t.Fatalf("unexpected Edelweiss price: %+v", price)
	}
}

func TestJobServiceImportScheduledMarketDataSupportsAliceBlueHoldingsCSV(t *testing.T) {
	db := testDB(t)
	ctx := context.Background()
	org := domain.Organization{Name: "Acme Alice Blue", BaseCurrency: "INR", CountryCode: "IN", FiscalYearStartMonth: 4}
	if err := db.Create(&org).Error; err != nil {
		t.Fatalf("create organization: %v", err)
	}

	tmpDir := t.TempDir()
	path := filepath.Join(tmpDir, "aliceblue.csv")
	if err := os.WriteFile(path, []byte("Symbol,ISIN,Date,LTP,Quantity\nTCS,INE467B01029,2026-07-31,4012.30,3\n"), 0o600); err != nil {
		t.Fatalf("write Alice Blue CSV: %v", err)
	}

	result, err := NewJobService(db).ImportScheduledMarketData(ctx, MarketDataImportJobInput{
		OrganizationID: org.ID,
		Path:           path,
		Format:         "aliceblue_holdings_csv",
	})
	if err != nil {
		t.Fatalf("ImportScheduledMarketData() error = %v", err)
	}
	...
	if price.PriceMinor != 401230 || price.Source != "aliceblue_holdings_csv" {
		t.Fatalf("unexpected Alice Blue price: %+v", price)
	}
}

func TestJobServiceImportScheduledMarketDataSupportsSamcoHoldingsCSV(t *testing.T) {
	db := testDB(t)
	ctx := context.Background()
	org := domain.Organization{Name: "Acme Samco", BaseCurrency: "INR", CountryCode: "IN", FiscalYearStartMonth: 4}
	if err := db.Create(&org).Error; err != nil {
		t.Fatalf("create organization: %v", err)
	}

	tmpDir := t.TempDir()
	path := filepath.Join(tmpDir, "samco.csv")
	if err := os.WriteFile(path, []byte("Symbol,ISIN,Date,LTP,Quantity\nSUNPHARMA,INE044A01036,2026-07-31,1675.40,6\n"), 0o600); err != nil {
		t.Fatalf("write Samco CSV: %v", err)
	}

	result, err := NewJobService(db).ImportScheduledMarketData(ctx, MarketDataImportJobInput{
		OrganizationID: org.ID,
		Path:           path,
		Format:         "samco_holdings_csv",
	})
	if err != nil {
		t.Fatalf("ImportScheduledMarketData() error = %v", err)
	}
	...
	var price domain.InvestmentPrice
	if err := db.Where("organization_id = ? AND symbol = ?", org.ID, "SUNPHARMA").First(&price).Error; err != nil {
		t.Fatalf("load Samco price: %v", err)
	}
	if price.PriceMinor != 167540 || price.Source != "samco_holdings_csv" {
		t.Fatalf("unexpected Samco price: %+v", price)
	}
}

func TestJobServiceImportScheduledMarketDataSupportsChoiceHoldingsCSV(t *testing.T) {
	db := testDB(t)
	ctx := context.Background()
	org := domain.Organization{Name: "Acme Choice", BaseCurrency: "INR", CountryCode: "IN", FiscalYearStartMonth: 4}
	if err := db.Create(&org).Error; err != nil {
		t.Fatalf("create organization: %v", err)
	}

	tmpDir := t.TempDir()
	path := filepath.Join(tmpDir, "choice.csv")
	if err := os.WriteFile(path, []byte("Symbol,ISIN,Date,LTP,Quantity\nULTRACEMCO,INE481G01011,2026-07-31,11234.55,1\n"), 0o600); err != nil {
		t.Fatalf("write Choice CSV: %v", err)
	}

	result, err := NewJobService(db).ImportScheduledMarketData(ctx, MarketDataImportJobInput{
		OrganizationID: org.ID,
		Path:           path,
		Format:         "choice_holdings_csv",
	})
	if err != nil {
		t.Fatalf("ImportScheduledMarketData() error = %v", err)
	}
	...
	var price domain.InvestmentPrice
	if err := db.Where("organization_id = ? AND symbol = ?", org.ID, "ULTRACEMCO").First(&price).Error; err != nil {
		t.Fatalf("load Choice price: %v", err)
	}
	if price.PriceMinor != 1123455 || price.Source != "choice_holdings_csv" {
		t.Fatalf("unexpected Choice price: %+v", price)
	}
}
...
func TestJobServiceImportScheduledMarketDataSupportsJainamHoldingsCSV(t *testing.T) {
	db := testDB(t)
	ctx := context.Background()
	org := domain.Organization{Name: "Acme Jainam", BaseCurrency: "INR", CountryCode: "IN", FiscalYearStartMonth: 4}
	if err := db.Create(&org).Error; err != nil {
		t.Fatalf("create organization: %v", err)
	}

	tmpDir := t.TempDir()
	path := filepath.Join(tmpDir, "jainam.csv")
	if err := os.WriteFile(path, []byte("Symbol,ISIN,Date,LTP,Quantity\nPOWERGRID,INE752E01010,2026-07-31,298.65,20\n"), 0o600); err != nil {
		t.Fatalf("write Jainam CSV: %v", err)
	}

	result, err := NewJobService(db).ImportScheduledMarketData(ctx, MarketDataImportJobInput{
		OrganizationID: org.ID,
		Path:           path,
		Format:         "jainam_holdings_csv",
	})
	if err != nil {
		t.Fatalf("ImportScheduledMarketData() error = %v", err)
	}
	...
	var price domain.InvestmentPrice
	if err := db.Where("organization_id = ? AND symbol = ?", org.ID, "POWERGRID").First(&price).Error; err != nil {
		t.Fatalf("load Jainam price: %v", err)
	}
	if price.PriceMinor != 29865 || price.Source != "jainam_holdings_csv" {
		t.Fatalf("unexpected Jainam price: %+v", price)
	}
}
