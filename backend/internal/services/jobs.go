...
func (s JobService) ImportScheduledMarketData(ctx context.Context, input MarketDataImportJobInput) (MarketDataImportJobResult, error) {
	...
	for _, organization := range organizations {
		...
		case "samco_holdings_csv":
			source := input.Source
			if source == "" {
				source = "samco_holdings_csv"
			}
			importResult, err = investments.ImportSamcoHoldingsCSV(ctx, ImportInvestmentPricesInput{
				OrganizationID: organization.ID,
				CSV:            string(payload),
				Source:         source,
			})
		case "choice_holdings_csv":
			source := input.Source
			if source == "" {
				source = "choice_holdings_csv"
			}
			importResult, err = investments.ImportChoiceHoldingsCSV(ctx, ImportInvestmentPricesInput{
				OrganizationID: organization.ID,
				CSV:            string(payload),
				Source:         source,
			})
		case "religare_holdings_csv":
			source := input.Source
			if source == "" {
				source = "religare_holdings_csv"
			}
			importResult, err = investments.ImportReligareHoldingsCSV(ctx, ImportInvestmentPricesInput{
				OrganizationID: organization.ID,
				CSV:            string(payload),
				Source:         source,
			})
		case "jainam_holdings_csv":
			source := input.Source
			if source == "" {
				source = "jainam_holdings_csv"
			}
			importResult, err = investments.ImportJainamHoldingsCSV(ctx, ImportInvestmentPricesInput{
				OrganizationID: organization.ID,
				CSV:            string(payload),
				Source:         source,
			})
